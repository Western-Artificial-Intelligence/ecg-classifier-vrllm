...
def transformer_encoder_block(inputs, num_heads, key_dim, dropout_rate):
    ...
    # Get the sequence length from the input shape (known at build time)
    seq_length = inputs.shape[1]  # This gets the static shape
    d_model = inputs.shape[2]  # Get the feature dimension
...
def plot(history):
    ...
    fig.tight_layout()
    plt.savefig(os.path.join(results_dir, 'hist.png'))
    plt.show()

    # save prediction score
    y_score = model.predict(x_test)
    output = pd.DataFrame({"y_true": y_test[:, 1], "y_score": y_score[:, 1], "subject": groups_test})
    output.to_csv(os.path.join(results_dir, "CNN-Transformer-LSTM.csv"), index=False)
    ...
    plt.savefig(os.path.join(results_dir, 'Confusion Matrix.png'), bbox_inches='tight', dpi=300)
    plt.show()
    ...
    plt.savefig(os.path.join(results_dir, 'ROC Curve.png'), bbox_inches='tight', dpi=300)
    plt.show()
...
if __name__ == "__main__":
    ...
    checkpoint = tf.keras.callbacks.ModelCheckpoint(filepath=os.path.join(models_dir, 'model.keras'), monitor='val_loss', verbose=1,
                                                    save_best_only=True)
    ...
    csv_logger = tf.keras.callbacks.CSVLogger(os.path.join(results_dir, 'log.csv'), separator=',', append=True)
    ...
    model.save(os.path.join(models_dir, "model.final.h5"))
